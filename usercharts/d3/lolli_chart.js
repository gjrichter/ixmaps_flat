...
(function () {
    ...
    ixmaps.lolliChart_init = function (SVGDocument, args) {
    ...
    ixmaps.lolliChart = function (SVGDocument, args) {
        ...
        // 2. get the color - defined directly or by class
 		...
 		var szColor = args.color || args.item.szColor || (args.theme.colorScheme[args.class]);
        ...
        if (nValue >= 0) {
            if (nHeight > 100 && args.theme.szFlag.match(/SHADOW/)) {
                svg.append("path")
                    .attr("d", "M60.686,0C27.17,0,0,27.17,0,60.685c0,30.571,22.609,55.851,52.016,60.059v259.84c0,4.309,3.493,7.802,7.803,7.802		c4.309,0,7.802-3.493,7.802-7.802v-259.62c30.248-3.443,53.75-29.11,53.75-60.279C121.371,27.17,94.201,0,60.686,0z")
                    .attr("style", "fill:#888888;stroke:#444444;stroke-width:1;fill-opacity:" + 1 + ";opacity:" + 0.2 + ";")
                    .attr("transform", "translate(" + nHeight / 6.4 + " " + nHeight / 17 + ") scale(" + nHeight / 2000 + " " + nHeight / 4000 + ") rotate(135)");
            }
            svg.append("path")
                .attr("d", "M60.686,0C27.17,0,0,27.17,0,60.685c0,30.571,22.609,55.851,52.016,60.059v259.84c0,4.309,3.493,7.802,7.803,7.802		c4.309,0,7.802-3.493,7.802-7.802v-259.62c30.248-3.443,53.75-29.11,53.75-60.279C121.371,27.17,94.201,0,60.686,0z")
                ...
                .attr("transform", "scale(" + nHeight / 2000 + " " + nHeight / 2000 + ") translate(" + -55 + " " + -380 + ") ");
        } else
			...
			if ( args.flag.match(/ZOOM/) || (
                ...
                if (args.flag.match(/VALUES/)) {
                    if (nValue > 0) {
                        svg.append("text")
                            .attr("x", 0)
                            .attr("y", -nHeight / 6.1 - nFontSize * 0.7)
                            .attr("style", "font-family:arial;font-size:" + nFontSize + "px;text-anchor:middle;fill:none;fill-opacity:" + szTextOpacity + ";stroke:white;stroke-width:" + nFontSize / 4 + "px;opacity:" + 0.5 + ";pointer-events:none")
                            .text("+" + szText);
                        svg.append("text")
                            .attr("x", 0)
                            .attr("y", -nHeight / 6.1 - nFontSize * 0.7)
                    ...
                    if (!args.theme.szFlag.match(/NONEGATIVE/)) {
                        if (args.theme.szFlag.match(/NEGATIVELIKEPOSITIVE/)) {
                            svg.append("text")
                                .attr("x", 0)
