...
(function () {
    ...
    ixMap.Query.prototype.xgetThemes = function () {
        if (this.themesA.length === 0 && this.map.SVGDocument) {
            var mapNode = this.map.SVGDocument.getElementById("mapzoomandpan");
            if (mapNode) {
    ...
    ixMap.Query.prototype.getThemes = function () {
        if (this.themesA.length === 0 && this.map.SVGDocument) {
            var themeNodesA = this.map.SVGDocument.getElementsByTagNameNS(szMapNs, "theme");
            if (themeNodesA) {
    ...
    ixMap.Query.prototype.getThemesWithInfo = function () {
        if (this.infoThemesA.length === 0 && this.map.SVGDocument) {
            var themeNodesA = this.map.SVGDocument.getElementsByTagNameNS(szMapNs, "theme");
            if (themeNodesA) {
                for (var i = 0; i < themeNodesA.length; i++) {
    ...
    ixMap.Query.prototype.getFieldsOfTheme = function (szTheme) {
        ...
        if (this.map.SVGDocument) {
            var themeNode = this.map.SVGDocument.getElementById(szTheme);
            if (themeNode == null) {
                if (map.Tiles && map.Tiles.nCount > 0) {
                    for (var y = 0;
                        (y < map.Tiles.nCount) && (themeNode == null); y++) {
                        for (var x = 0;
                            (x < map.Tiles.nCount) && (themeNode == null); x++) {
                            themeNode = this.map.SVGDocument.getElementById(szTheme + "#" + "00" + String(y) + "00" + String(x));
                        }
                    }
                }
            }
    ...
    ixMap.Query.prototype.getValueOfFieldAndItem = function (szId, szField) {
        ...
        if (this.map.SVGDocument) {
            var itemNode = this.map.SVGDocument.getElementById(szId);
            if (itemNode == null) {
    ...
    ixMap.Query.prototype.addFoundNode = function (nodeObj, szFeature, szItem, itemA) {
        ...
        if (szTheme && szTheme.length && szId && szId.length) {
            ...
            this.foundNodesA[this.foundNodesA.length] = {
                node: nodeObj,
                id: szId,
                theme: szTheme,
                feature: szFeature,
                        ...
                        for (var p = 0; p < anzParts; p++) {
                            ...
                            for (var c = 0; c < anzConditions; c++) {
                                ...
                                switch (thisQuery.partA[p].conditionA[c].szOperator) {
                                    case '<':
                                        fOK = (Number(infoA[nIndex]) < thisQuery.partA[p].conditionA[c].szValue);
                                        break;
                                    case '==':
                                        fOK = ((infoA[nIndex]) == thisQuery.partA[p].conditionA[c].szValue);
                                        break;
                                    case '>':
                                        fOK = (Number(infoA[nIndex]) > thisQuery.partA[p].conditionA[c].szValue);
                                        break;
    ...
    ixMap.Query.prototype.searchItemByAttribute = function (szAttribute, szValue) {
        ...
        childsA = map.SVGRootElement.getElementsByTagName('use');
        for (i = 0; i < childsA.length; i++) {
            nodeA[nodeA.length] = childsA.item(i);
        }
        childsA = map.SVGRootElement.getElementsByTagName('g');
        for (i = 0; i < childsA.length; i++) {
            nodeA[nodeA.length] = childsA.item(i);
        }
        for (i = 0; i < nodeA.length; i++) {
    ...
    ixMap.Query.prototype.execGotoFoundItem = function (nIndex, mode, newZoom) {
        ...
        if (nIndex < 0) {
            ...
            if (mode == 'zoomto') {
                var zoomNode = this.map.SVGDocument.getElementById("mapzoomandpan");
                if (zoomNode) {
                ...
                for (i = 0; i < this.foundNodesA.length; i++) {
                    ...
                    if (!mode.match(/info/) || !map.Themes.isNodePartOfAnyTheme(this.foundNodesA[i].node)) {
                        ...
                        for (var n = 0; n < shapeNodesA.length; n++) {
                            map.highLightList.addItem(shapeNodesA[n]);
                        }
                        map.highLightList.startToggle();
                    }
                    if (mode.match(/info/)) {
     ...
     */
    var _QueryCondition = function(szTopic, szOperator, szValue) {
