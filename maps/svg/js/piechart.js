...
(function () {
    ...
    DonutChart.prototype.realize = function () {
        ...
        if (this.szStyle.match(/SYMMETRIC/)) {
            ...
            if (this.szStyle.match(/STARBURST/)) {
                ...
                if (this.szStyle.match(/BIGTOTOP/)) {
                    nEndAngle = 360;
                    for (var x = 0;
                        (x < this.partsA.length) && (this.partsA[x] != biggestPart); x++) {
        ...
        for (i = 0; i < this.partsA.length; i++) {
            ...
            if (this.szStyle.match(/DIRECTION/)) {
                if (this.partsA[i].nAngle) {
                    this.donutPartsA[i] = {
                        nStartAngle: this.partsA[i].nAngle - 1,
                        nEndAngle: this.partsA[i].nAngle + 1,
                        color: this.partsA[i].color,
                        nHeight: this.partsA[i].nHeight,
                        nOffset: this.partsA[i].nOffset
                    };
                } else {
                    this.donutPartsA[i] = {};
                }
            ...
            if (this.szStyle.match(/DIRECTED/)) {
                if (this.partsA[i].nAngle) {
                    this.donutPartsA[i] = {
                        nStartAngle: this.partsA[i].nAngle - 10,
                        nEndAngle: this.partsA[i].nAngle + 10,
                        color: this.partsA[i].color,
                        nHeight: this.partsA[i].nHeight,
                        nOffset: this.partsA[i].nOffset
                    };
                ...
                this.donutPartsA[i] = {
                    nStartAngle: nStartAngle + nGapAngle,
                    nEndAngle: nEndAngle - nGapAngle,
                    color: this.partsA[i].color,
                    nHeight: this.partsA[i].nHeight,
                    nOffset: this.partsA[i].nOffset
                };
        ...
        if (this.szStyle.match(/STARBURST/) && this.szStyle.match(/POLAR/)) {
            ...
            c.setAttributeNS(szMapNs, "tooltip", "range " + String((this.nMaxValue || 100) * 0.75) + "-" + String((this.nMaxValue || 100) * 1));
            ...
            c.setAttributeNS(szMapNs, "tooltip", "range " + String((this.nMaxValue || 100) * 0.50) + "-" + String((this.nMaxValue || 100) * 0.75));

            var radius = Math.max(1, this.nRadInner + (this.nRadOuter - this.nRadInner) * (Math.sqrt(nMaxRadius50) / Math.sqrt(this.nMaxValue)) * scale);
            c = map.Dom.newShape('circle', this.targetGroup, this.mX, this.mY, radius, "fill:none;stroke:#444444;stroke-width:1px;");
            c.setAttributeNS(szMapNs, "tooltip", "range " + String((this.nMaxValue || 100) * 0.25) + "-" + String((this.nMaxValue || 100) * 0.50));

            var radius = Math.max(1, this.nRadInner + (this.nRadOuter - this.nRadInner) * (Math.sqrt(nMaxRadius25) / Math.sqrt(this.nMaxValue)) * scale);
            c = map.Dom.newShape('circle', this.targetGroup, this.mX, this.mY, radius, "fill:none;stroke:#444444;stroke-width:3px;stroke-dasharray:50 50");
            c.setAttributeNS(szMapNs, "tooltip", "range " + String((this.nMaxValue || 100) * 0) + "-" + String((this.nMaxValue || 100) * 0.25));
    ...
    function _crc_newText(targetDocument, targetGroup, x, y, s, szText) {
        ...
        var nT = _crc_constructNode('text', targetDocument, targetGroup, {
            x: String(x),
            y: String(y),
            style: s
        });
    ...
    window.DonutCharts = new _DonutCharts();
