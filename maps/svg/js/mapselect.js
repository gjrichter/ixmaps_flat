...
(function () {
    ...
    **/
    ...
    MapSelection.prototype.selectShapes = function (startIndex) {
        ...
        if (map.HTMLWindow.ixmaps.htmlgui_onSelection(this.szId)) {
            return null;
        }
    ...
    MapSelection.prototype.showInfo = function (fDone) {
        ...
        if (this.widgetNode) {
            ...
            var newInfo = new InfoContainer(map.SVGDocument, map.SVGFixedGroup, szDisplayId + ":movable", this.selectionCenter, new point(-map.Scale.normalX(55), map.Scale.normalY(50)), "fixed|pointer", szTitle);
        }
        // container backgrond gray
        newInfo.frameNode.style.setProperty("fill", "#e8e8e8", "");
        var temp = map.szTextGridStyle;
        map.szTextGridStyle = "firstgray";
        ...
        if (this.szSelectShape == "activeBuffer") {
            ...
            var textA = new Array(map.Dictionary.getLocalText("layer"), map.Dictionary.getLocalText("selected by"), map.Dictionary.getLocalText("selected items"), szLayer, this.szSelectQuery, String(this.nSelected) + " / " + String(this.nThemeItems) + "  (" + String(__formatValue(100 / this.nThemeItems * this.nSelected, 2)) + "%" + ")");
            var styleA = new Array(3);
            styleA[0] = "font-weight:bold;fill:#ffffff;";
            styleA[1] = "font-weight:bold;fill:#ffffff;";
            styleA[2] = "font-weight:bold;fill:#ffffff;";
            ...
            var textA = new Array(map.Dictionary.getLocalText("layer"), map.Dictionary.getLocalText("selected items"), "(generic)", String(this.nSelected) + " / " + String(this.nThemeItems) + "  (" + String(__formatValue(100 / this.nThemeItems * this.nSelected, 2)) + "%" + ")");
            var styleA = new Array(2);
            styleA[0] = "font-weight:bold;fill:#ffffff;";
            styleA[1] = "font-weight:bold;fill:#ffffff;";
            nLineOff += 40;
        } else {
            var textA = new Array(map.Dictionary.getLocalText("layer"), map.Dictionary.getLocalText("selected items"), this.szThemesA.length == 1 ? String(map.Layer.listA[this.szThemesA[0]].szLegendName) : "(multiple)", String(this.nSelected) + " / " + String(this.nThemeItems) + "  (" + String(__formatValue(100 / this.nThemeItems * this.nSelected, 2)) + "%" + ")");
            var styleA = new Array(2);
            styleA[0] = "font-weight:bold;fill:#ffffff;";
            styleA[1] = "font-weight:bold;fill:#ffffff;";
            nLineOff += 40;
        ...
        var newText = createTextGrid(map.SVGDocument, gridGroup, szDisplayId + ":textgrid", textA, 2, 11, styleA, "fill:#bbbbbb;");
        ...
        if (this.activeTheme) {
            ...
            if (this.nValuesSumA.length == 1) {
                ...
                //
                if (this.activeTheme.szFlag.match(/CATEGORICAL/)) {
                    var textA = new Array(map.Dictionary.getLocalText("selected items"), String(__formatValue(Number(this.nCount, 2))));
                    var styleA = new Array(2);
                    styleA[0] = "font-weight:bold;fill:#7996D7;";
                    styleA[1] = "font-weight:bold;fill:#000000;font-size:240px;";
                    createTextGrid(map.SVGDocument, gridGroup, szDisplayId + ":textgrid", textA, 2, 11, styleA);
                    ...
                    var textA = new Array(map.Dictionary.getLocalText("value of selection"), map.Dictionary.getLocalText("% of total value"), " ", map.Dictionary.getLocalText("min"), map.Dictionary.getLocalText("max"), map.Dictionary.getLocalText("average"), String(__formatValue(Number(this.itemA["selection"].nValuesA[0]), 2)) + " " + this.szUnit, String(__formatValue(Number(this.nValuesSumA[0]) / Number(this.activeTheme.nOrigSumA[0]) * 100, 2)) + " %", " ", String(__formatValue(Number(this.nValuesMinA[0]), 2)) + " " + this.szUnit, String(__formatValue(Number(this.nValuesMaxA[0]), 2)) + " " + this.szUnit, String(__formatValue(Number(this.nValuesSumA[0]) / this.nCount, 2)) + " " + this.szUnit);
                    ...
                    var donut = DonutCharts.newChart(map.SVGDocument, chartGroup, map.Scale.normalX(55), map.Scale.normalY(55), 0, 0);
                    //donut.setStyle("3D");
                    donut.setRadInner(0);
                ...
                if (this.activeTheme.szAggregation.match(/sum/) || this.activeTheme.szFlag.match(/SUM/)) {
                    ...
                    var textA = new Array(map.Dictionary.getLocalText("total of selection"), String(__formatValue(nSelectionTotal, 2)) + " (" + String(__formatValue(Number(nSelectionTotal) / Number(nTotal) * 100, 2)) + " %)");
                    var styleA = new Array(2);
                    styleA[0] = "font-weight:bold;fill:#7996D7;";
                    styleA[1] = "font-weight:bold;fill:#000000;font-size:240px;";
                    createTextGrid(map.SVGDocument, gridGroup, szDisplayId + ":textgrid", textA, 2, 11, styleA);
    ...
    ixMap.Selections.prototype.selectItemByPosition = function (szThemes, pos) {
        ...
        for (var i in themeObj.itemA) {
            var bufferNode = map.SVGDocument.getElementById(i);
            var nodesA = bufferNode.childNodes;
            for (var r = 0; r < nodesA.length; r++) {
                var ptPointA = map.Api.getShapePoints(nodesA.item(r));
                let fSelected = __isPointInPoly(position, ptPointA);
                if (fSelected) {
                    return i;
                }
            }
    ...
    ixMap.Selections.prototype.selectItemByBoundingBox = function (szThemes, box) {
        ...
        let NE = map.Scale.getMapPositionOfLatLon(box[1].lat, box[1].lng);
        let selected = [];
        for (var i in themeObj.itemA) {
            var bufferNode = map.SVGDocument.getElementById(i);
            var nodesA = bufferNode.childNodes;
            for (var r = 0; r < nodesA.length; r++) {
                var ptPointA = map.Api.getShapePoints(nodesA.item(r));
                for (var p in ptPointA) {
                    if (ptPointA[p].x > SW.x &&
                        ptPointA[p].x < NE.x &&
                        ptPointA[p].y < SW.y &&
                        ptPointA[p].y > NE.y)
                        selected.push(i);
