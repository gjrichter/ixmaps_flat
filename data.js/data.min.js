...
0,d=b.length;e<d;e++)c[String(b[e])]=c[String(b[e])]?c[String(b[e])]+", "+a[e]:a[e];return c},lookup:function(a,b){var c=b.value;b=b.lookup;var e=c+"_"+b;this.lookupsA&&this.lookupsA[e]||(this.lookupsA=this.lookupsA||[],this.lookupsA[e]=this.lookupArray(c,b));return this.lookupsA[e][a]||"-"},toKeyValue:function(a){return this.lookupArray(a.value,a.key)},addColumn:function(a,b){if(!a.destination)return alert("'data.addColumn' no destination defined!"),null;var c=null;if(a.source){for(var e=0,d=this.fields.length;e<
d;e++)this.fields[e].id==a.source&&(c=e);if(null==c)return alert("'data.addColumn' source column '"+a.source+"' not found!"),null}this.fields.push({id:String(a.destination),created:!0});this.table.fields++;if(b&&"function"==typeof b)for(a=0,d=this.records.length;a<d;a++)this.records[a].push(null!=c?b(this.records[a][c],this.records[a]):b(this.records[a]));else if(b&&"object"==typeof b)for(a=0,d=this.records.length;a<d;a++)this.records[a].push(b[a]||0);else if(a.values&&"object"==typeof a.values)for(b=
0,d=this.records.length;b<d;b++)this.records[b].push(a.values[b]||0);else for(b=0,d=this.records.length;b<d;b++)this.records[b].push(0);return this},addRow:function(a){if(!a||"object"!==typeof a)return alert("'data.addRow' no options defined!"),null;for(var b=[],c=0,e=this.fields.length;c<e;c++)b.push("");for(var d in a)this.column(d)?b[this.column(d).index]=a[d]:alert("'data.addRow' column '"+d+"' not found!");this.records.push(b);this.table.records++;return this},filter:function(a){this.selection=
new h.Table;for(var b in this.records)a&&a(this.records[b])&&(this.selection.records.push(this.records[b]),this.selection.table.records++);this.selection.fields=this.fields.slice();this.selection.table.fields=this.table.fields;return this.selection},select:function(a){if(a.match(/WHERE/)){for(var b=a.split("WHERE")[1].trim().split(" "),c=0;c<b.length;c++)if(b[c].length){if('"'==b[c][0]&&'"'!=b[c][b[c].length-1]){do b[c]=b[c]+" "+b[c+1],b.splice(c+1,1);while('"'!=b[c][b[c].length-1])}if("("==b[c][0]&&
")"!=b[c][b[c].length-1]){do b[c]=b[c]+" "+b[c+1],b.splice(c+1,1);while(")"!=b[c][b[c].length-1])}}else b.splice(c,1),c--;this.filterQueryA=[];c={};var e="";do{var d=0;3<=b.length&&(c={},c.szSelectionField=b[0].replace(/("|)/g,""),c.szSelectionOp=b[1],c.szSelectionValue=b[2].replace(/("|)/g,""),d=3);"BETWEEN"==c.szSelectionOp&&5<=b.length&&"AND"==b[3]&&(c.szSelectionValue2=b[4],d=5);if(d){for(var f=0;f<this.fields.length;f++)this.fields[f].id==c.szSelectionField&&(c.nFilterFieldIndex=f),"$"+this.fields[f].id+
"$"==c.szSelectionValue&&(c.nFilterValueIndex=f);c.szCombineOp=e;this.filterQueryA.push(c);b.splice(0,d)}else{q("data.js - selection error - incomplete query!\nquery: "+a);break}if(b.length&&"AND"==b[0])e="AND",b.splice(0,1);else if(b.length&&"OR"==b[0])e="OR",b.splice(0,1);else break}while(b.length);this.selection=new h.Table;for(b=0;b<this.filterQueryA.length;b++)if("undefined"===typeof this.filterQueryA[b].nFilterFieldIndex)return this.selection.fields=this.fields.slice(),this.selection.table.fields=
this.table.fields,_LOG("Selection: invalid query: "+a),this.selection;a=0;for(b=this.records.length;a<b;a++){c=null;e=0;for(d=this.filterQueryA.length;e<d;e++)f=!0,this.__szValue=String(this.records[a][this.filterQueryA[e].nFilterFieldIndex]),this.__szSelectionOp=this.filterQueryA[e].szSelectionOp.toUpperCase(),this.__szSelectionValue=this.filterQueryA[e].szSelectionValue,this.__szSelectionValue2=this.filterQueryA[e].szSelectionValue2,this.__szCombineOp=this.filterQueryA[e].szCombineOp,null!=this.filterQueryA[e].nFilterValueIndex&&
(this.__szSelectionValue=String(this.records[a][this.filterQueryA[e].nFilterValueIndex])),f=__scanValue(this.__szValue),"="==this.__szSelectionOp?f="*"==this.__szSelectionValue?""!=this.__szValue.replace(/ /g,""):this.__szValue==this.__szSelectionValue||f==Number(this.__szSelectionValue):"<>"==this.__szSelectionOp?f=!(this.__szValue==this.__szSelectionValue||f==Number(this.__szSelectionValue)):">"==this.__szSelectionOp?f=f>Number(this.__szSelectionValue):"<"==this.__szSelectionOp?f=f<Number(this.__szSelectionValue):
">="==this.__szSelectionOp?f=f>=Number(this.__szSelectionValue):"<="==this.__szSelectionOp?f=f<=Number(this.__szSelectionValue):"LIKE"==this.__szSelectionOp?"*"==this.__szSelectionValue?f=this.__szValue.length:(f=this.__szSelectionValue.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),f=(new RegExp(f,"i")).test(this.__szValue)):"NOT"==this.__szSelectionOp?(f=this.__szSelectionValue.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),f=!(new RegExp(f,"i")).test(this.__szValue)):"IN"==this.__szSelectionOp?f=this.__szSelectionValue.split(",").map(function(a){return a.trim()}).includes(this.__szValue):
"BETWEEN"==this.__szSelectionOp?f=f>=Number(this.__szSelectionValue)&&f<=Number(this.__szSelectionValue2):(f=this.__szSelectionValue.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),f=(new RegExp(f,"i")).test(this.__szValue)),c="AND"==this.__szCombineOp?c&&f:c||f;c&&(this.selection.records.push(this.records[a].slice()),this.selection.table.records++)}}this.selection.fields=this.fields.slice();this.selection.table.fields=this.table.fields;return this.selection},aggregate:function(a,b){var c=!1;a.lead&&(c=a.calc&&
"mean"==a.calc,b=a.lead,a=a.column||a.value);b=b.split("|");for(var e=[],d=null,f=0;f<b.length;f++)for(var g=0;g<this.fields.length;g++)this.fields[g].id==b[f]&&(e[f]=g),this.fields[g].id==a&&(d=g);this.aggregation=new h.Table;xRecords=[];xCount=[];g=0;for(f=this.records.length;g<f;g++){for(var k=[],l=0,p=e.length;l<p;l++)k.push(this.records[g][e[l]]);k=k.join("");if(xRecords[k])xRecords[k][e.length]+=__scanValue(this.records[g][d]),xCount[k][e.length]++;else{xRecords[k]=[];xRecords[k][e.length]=
__scanValue(this.records[g][d]);for(l=0;l<e.length;l++)xRecords[k][l]=this.records[g][e[l]];xCount[k]=[];xCount[k][e.length]=1}}d=0;for(f=xRecords.length;d<f;d++)c&&(xRecords[d][e.length]/=xCount[d][e.length]),this.aggregation.records.push(xRecords[d]),this.aggregation.table.records++;c=[];for(e=0;e<b.length;e++)c[e]={id:b[e]};c[b.length]={id:a};this.aggregation.fields=c;this.aggregation.table.fields=c;return this.aggregation},condense:function(a,b){var c={},d=[];a&&a.lead&&(b=a,a=b.lead);a=this.columnIndex(a);
if(b&&b.keep)if("string"==typeof b.keep)d[this.columnIndex(b.keep)]=!0;else for(i=0;i<b.keep.length;i++)d[this.columnIndex(b.keep[i])]=!0;for(var m=[],f=0;f<this.records.length;f++){var g=String(this.records[f][a]);if(null!=c[g]){g=c[g];for(var k=0,l=this.records[f].length;k<l;k++)if(!d[k])if(!isNaN(this.records[f][k]))m[g][k]=b&&"max"==b.calc?Math.max(Number(m[g][k]),Number(this.records[f][k])):Number(m[g][k])+Number(this.records[f][k]);else if(isNaN(this.records[f][k])&&m[g][k]!=this.records[f][k]){var p=
parseFloat(String(m[g][k]).split(" (+")[1])||0;m[g][k]=String(m[g][k]).split(" (+")[0]+" (+"+ ++p+") "}}else m.push(this.records[f].slice()),c[g]=m.length-1}this.__condense=new h.Table;this.__condense.fields=this.fields;this.__condense.table.fields=this.fields;this.__condense.records=m.slice();this.__condense.table.records=this.__condense.records.length;return this.__condense},groupColumns:function(a){for(var b=a.source,c=[],d=0,h=b.length;d<h;d++)c[d]=this.column(b[d]).index;this.addColumn({destination:a.destination},
function(a){for(var b=0,d=0,e=c.length;d<e;d++)b+=Number(a[c[d]]);return b});return this},pivot:function(a){a.lead=a.lead||a.rows;a.cols=a.cols||a.columns;a.keep=a.keep||[];a.sum=a.sum||[];a.lead=w(a.lead);a.cols=w(a.cols);a.keep=w(a.keep);a.sum=w(a.sum);a.value=w(a.value);a.forced=w(a.forced);for(var b=[],c=0;c<this.fields.length;c++)b[String(this.fields[c].id)]=c;c=0;for(var d=a.lead.length;c<d;c++)"undefined"==typeof b[a.lead[c]]&&q("data.pivot - pivot keep column '"+a.lead[c]+"' not found");c=
0;for(d=a.cols.length;c<d;c++)a.cols&&"undefined"==typeof b[a.cols[c]]&&q("data.pivot - pivot columns source column '"+a.cols[c]+"' not found");c=0;for(d=a.keep.length;c<d;c++)"undefined"==typeof b[a.keep[c]]&&q("data.pivot - pivot keep column '"+a.keep[c]+"' not found");c=0;for(d=a.sum.length;c<d;c++)"undefined"==typeof b[a.sum[c]]&&q("data.pivot - pivot sum column '"+a.sum[c]+"' not found");c=0;for(d=a.value.length;c<d;c++)"undefined"==typeof b[a.value[c]]&&q("data.pivot - pivot value column '"+
a.value[c]+"' not found");c=[];var m=[],f=this.records;if(a.forced)for(d=0;d<a.forced.length;d++)m[String(a.forced[d])]=0;var g=0;for(d=f.length;g<d;g++){for(var k=[f[g][b[a.lead[0]]]],l=1;l<a.lead.length;l++)k.push(f[g][b[a.lead[l]]]);k=k.join("|");l=String(f[g][b[a.cols[0]]]);if("string"==a.calc)var p=f[g][b[a.value[0]]];else if(p=1,a.value&&a.value.length)for(var n=p=0;n<a.value.length;n++)p+=a.value[n]?__scanValue(f[g][b[a.value[n]]]):1;if(!l||1>l.length)l="undefined";"undefined"==typeof m[l]&&
(m[l]=0);if(c[k]){for(n=0;n<a.keep.length;n++)f[g][b[a.keep[n]]]&&f[g][b[a.keep[n]]].length&&c[k][a.keep[n]]!=f[g][b[a.keep[n]]]&&(c[k][a.keep[n]]=f[g][b[a.keep[n]]]);for(n=0;n<a.sum.length;n++)c[k][a.sum[n]]+=Number(f[g][b[a.sum[n]]])}else{c[k]={Total:0};for(n=0;n<a.keep.length;n++)c[k][a.keep[n]]=f[g][b[a.keep[n]]];for(n=0;n<a.sum.length;n++)c[k][a.sum[n]]=Number(f[g][b[a.sum[n]]])}c[k].Total+=p;c[k][l]?"max"==a.calc?c[k][l]=Math.max(p,c[k][l]):(c[k][l]+=p,c[k][l+"count"]++):(c[k][l]=p,c[k][l+"count"]=
1)}this.__pivot=new h.Table;for(b=0;b<a.lead.length;b++)this.__pivot.fields.push({id:a.lead[b]});for(b=0;b<a.keep.length;b++)this.__pivot.fields.push({id:a.keep[b]});for(b=0;b<a.sum.length;b++)this.__pivot.fields.push({id:a.sum[b]});if(a.cols&&a.cols.length)for(var u in m)Object.prototype.hasOwnProperty.call(m,u)&&this.__pivot.fields.push({id:u});this.__pivot.fields.push({id:"Total"});for(var t in c)if(Object.prototype.hasOwnProperty.call(c,t)){u=[];b=t.split("|");if(a.lead&&a.lead.length)for(d=0;d<
b.length;d++)u.push(b[d]);for(b=0;b<a.keep.length;b++)u.push(c[t][a.keep[b]]);for(b=0;b<a.sum.length;b++)u.push(c[t][a.sum[b]]);if(a.cols&&a.cols.length)for(var r in m)Object.prototype.hasOwnProperty.call(m,r)&&("mean"==a.calc?u.push((c[t][r]||0)/(c[t][r+"count"]||1)):u.push(c[t][r]||0));u.push(c[t].Total);this.__pivot.records.push(u);this.__pivot.table.records++}return this.__pivot},subtable:function(a){this.__subt=new h.Table;if(a.fields){a.columns=[];for(var b=0;b<a.fields.length;b++)for(var c=
...
"").replace(/,/gi,".")):parseFloat(String(a).replace(/ /gi,""));return isNaN(a)?0:a};h.Table.prototype.addTimeColumns=function(a){if(!a.source)return null;for(var b in this.fields)if(this.fields[b].id==a.source){for(var c=a.create||["date","year","month","day","hour"],d=0;d<c.length;d++)this.fields.push({id:String(c[d])}),this.table.fields++;d=this.records.length;var h;for(h=0;h<d;h++)for(var f=new Date(this.records[h][b]),g=0;g<c.length;g++)switch(c[g]){case "date":var k=String(f.getDate())+"."+
String(f.getMonth()+1)+"."+String(f.getFullYear());this.records[h].push(k);break;case "year":this.records[h].push(f.getFullYear());break;case "month":this.records[h].push(f.getMonth()+1);break;case "day":this.records[h].push(f.getDay());break;case "hour":this.records[h].push(f.getHours())}}return this};h.Column=function(){this.valueA=this.index=this.table=null};h.Column.prototype={values:function(){this.valueA=[];for(var a in this.table.records)this.valueA.push(this.table.records[a][this.index]);
