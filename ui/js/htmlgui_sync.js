...
(function (window, document, undefined) {
	...
	* ------------------------------------------------------------------ */
	...
	ixmaps.syncSlaveMaps = function (ptSW, ptNE, nZoom) { retrun;
	...
	function __doSynchronizeSVGMap(fPanOnly) {
		...
		try {
			ixmaps.embeddedSVG.window._TRACE("htmlgui:\n Center = "
                                             + arrayPtLatLon[0].lat + ", "
                                             + arrayPtLatLon[0].lng + "\n"
                                             + "bCenter = "
                                             + ptBlat + ", "
		...
		//if(arrayPtLatLon[1].lng-arrayPtLatLon[0].lng > 360){
			while (arrayPtLatLon[0].lng < -360) {
				arrayPtLatLon[0].lng += 180;
			}
			while (arrayPtLatLon[1].lng > 360) {
				arrayPtLatLon[1].lng -= 180;
			}
		//}
