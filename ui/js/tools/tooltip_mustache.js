...
(function () {
	...
	$.when(
		...
		$.getScript(ixmaps.szResourceBase+"ui/js/tools/mustache.js",
	...
	ixmaps.htmlgui_onTooltipDisplay = function (evt, szText, szId) {
		...
		if (__fTooltipPin) {
			...
			szHtml += "<div onclick='ixmaps.htmlgui_deleteItemPinned()' style='position:absolute;top:-0.7em;right:-0.7em'><img src='"+ixmaps.szResourceBase+"ui/resources/images/delete.png' height='22px'></div>"
	...
	var __add_chart = function(themeObj,szItem){
		...
		if (!themeObj.szFlag.match(/TOOLTIPNOCHART/)) {
			...
			if (SVGBox && SVGBox.width && SVGBox.height) {
				...
				if (themeObj.szFlag.match(/CHOROPLETH/) && !themeObj.szFlag.match(/(BAR)|PLOT|DOMINANT|COMPOSECOLOR/)) {
                    width = 250; 
                } else
			...
			if (data) {
				...
				for (d = 0; d < Math.min(50, data.length); d++) {
					...
					if (data.length > 1) {
						...
						szHtml += "<tr><td colspan='2' style='font-size:1.5em;border:solid black 1px;border-radius:0.5em;padding:0.1em 0.4em 0em 0.3em'>"+(d+1)+"</td><tr>";
						szHtml += "<tr><td style='font-size:0.1em;'>&nbsp;</td><tr>";
					}
					...
					for (i in dataObject) {
						...
						if (szValue.match(/http:/) || szValue.match(/https:/)) {
							if (szValue.match(/.jpg/) || szValue.match(/.jpeg/) || szValue.match(/.png/)) {
								...
								szHtml += "<tr><td class='label'>" + i + "</td><td class='value clip'>" + szValue + "</td></tr>";
							} else {
							...
							if (!i.match(/------/)) {
								...
								if (i == themeObj.szSizeField ||
									...
									szHtml += "<tr><td class='label' style='text-align:right;color:#000000;' ><span style='color:black'>&larr;</span> <b>" + (i) + "</b></td><td class='value' style='color:" + highLight + ";background:" + highLightBg + ";text-align:left;'>" + szValue + suffix + "</td></tr>";
								} else {
								szHtml += "<tr><td class='label' style='text-align:right;color:#cccccc';' >" + (i) + "</td><td class='value' style='color:" + normal + ";text-align:left;'>" + szValue + suffix + "</td></tr>";
								}
							} else {}
						}
					}
