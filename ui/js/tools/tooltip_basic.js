...
(function () {
	...
	ixmaps.htmlgui_onTooltipDisplay = function (evt, szText, szId) {
		...
		yPos = Math.min((window.innerHeight-height-50),(yPos + 20));
			...
			if (themeObj.szFlag.match(/PLOT/)) {
				...
				for (var t = 0; t < themesA.length; t++) {
					...
					if (objTheme.szFlag.match(/PLOT/)) {
						objTheme.getChart(window.document.getElementById("getchartmenutarget"), szItem, 60, "VALUES|XAXIS|ZOOM|BOX|GRID");
					}
				}
			} else
	...
	ixmaps.htmlgui_onItemClick = function (evt, szId, shape) {
		...
		__fTooltipPinned = false;
		__fTooltipPin = true;
		return ixmaps.htmlgui_onItemOver(evt, szId, shape);
	}
	...
	ixmaps.onMouseOver = function (source) {
